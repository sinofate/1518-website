...
(function () {
  ...
  function kangxiDb() {
    return (typeof window !== "undefined" && window.KANGXI_DB) || {};
  }
  ...
  function hasStroke(char) {
    return KANGXI_STROKES[char] != null || shared().COMMON_STROKES?.[char] != null || kangxiDb()[char] != null;
  }
  ...
  function calcFiveGrids(fullName) {
    ...
    const strokes = allChars.map((char) => ({ char, stroke: fallbackStroke(char), element: elementByChar(char), source: hasStroke(char) ? "本地康熙笔画库" : "缺字估算" }));
